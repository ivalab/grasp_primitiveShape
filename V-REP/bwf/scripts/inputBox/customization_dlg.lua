...
function model.dlg.updateEnabledDisabledItems()
    if model.dlg.ui then
        ...
        simUI.setEnabled(model.dlg.ui,41,config.signalState,true)
        simUI.setEnabled(model.dlg.ui,42,not config.signalState,true)
    end
end
...
function model.dlg.createDlg()
    if (not model.dlg.ui) and simBWF.canOpenPropertyDialog() then
        ...
        <tabs id="77">
            ...
            <tab title="Signal">
            <group layout="form" flat="false">
                <label text="Manual trigger" style="* {font-weight: bold;}"/>  <label text=""/>
                <button text="Set to low"  on-click="model.dlg.manualTriggerLow_callback" id="41" style="* {min-width: 150px;}"/>
                <button text="Set to high"  on-click="model.dlg.manualTriggerHigh_callback" id="42" style="* {min-width: 150px;}"/>
            </group>
            </tab>
            <tab title="More">
            <group layout="form" flat="false">
