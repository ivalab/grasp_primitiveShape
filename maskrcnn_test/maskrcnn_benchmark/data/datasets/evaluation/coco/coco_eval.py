...
def select_top_predictions(predictions,confidence_threshold=0.45):
    """
...
def py_mask_nms(masks, scores, threshold=0.5):
...
def prepare_for_coco_detection(predictions, dataset):
    ...
    for image_id, prediction in enumerate(predictions):
        ...
        # ## Todo(Clark): NMS test
        # prediction_select=select_top_predictions(prediction,0.45)
        # boxes = prediction_select.bbox
        #
        # ## ROI
        # row, column = image_height,image_width
        # keep = []
        # idx = 0
        # for box in boxes:
        ...
        #             40 < bottom_right[0] and bottom_right[0] < row - 20 and 25 < bottom_right[1] and bottom_right[
        #                 1] < column - 40):
        #         idx = idx + 1
        #         continue
        #
        ...
        #
        #     idx = idx + 1
        # predictions_afterROI = prediction_select[torch.from_numpy(np.array(keep)).type(torch.int64)]
        #
        # ## NMS
        # masks = predictions_afterROI.get_field("mask").numpy()
        # scores = predictions_afterROI.get_field("scores")
        #
        # keep_nms = py_mask_nms(masks, scores, 0.4)
        # predictions_afterNMS = predictions_afterROI[keep_nms]
        # prediction=predictions_afterNMS
